...
function Card({ className, size = "default", ...props }: React.ComponentProps<"div"> & { size?: "default" | "sm" }) {
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "brutal-border brutal-shadow bg-card text-card-foreground group/card flex flex-col gap-4 overflow-hidden rounded-sm py-4 text-xs/relaxed has-[>img:first-child]:pt-0 data-[size=sm]:gap-3 data-[size=sm]:py-3 *:[img:first-child]:rounded-t-sm *:[img:last-child]:rounded-b-sm",
        className,
      )}
      {...props}
    />
